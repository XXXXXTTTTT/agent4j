...
- 三类项目知识文件均为可选来源；没有任何精确命名的知识文件时返回空上下文，不阻止普通外部项目运行。
- 组合上下文受 `maxTokens` 约束；存在根 `AGENTS.md` 时优先保留，且其本身超预算时明确失败；
  其余来源按加载优先级在完整文件边界内选择，不切断 UTF-8 或 Markdown 段落。
...
仅文件知识并写 `DEGRADED` 证据。未发现项目知识文件不是编译失败；只有已发现文件的路径、格式、
大小、行数或 token 预算违反边界时才终止。
