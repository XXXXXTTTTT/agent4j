...
`agent-sandbox` 当前不依赖 `agent-core`，因此 Task 2 不得让 Docker runner 反向依赖 core。现有三流生命周期 SPI `McpStdioProcess` 从 `agent-core/src/main/java/com/agent/core/tool/mcp/McpStdioProcess.java` 移至 `agent-sandbox/src/main/java/com/agent/sandbox/mcp/McpStdioProcess.java`；它只包含 `stdout()`、`stdin()`、`stderr()`、`isAlive()`、`destroy()`，不引入 docker-java、Spring 或 core 类型。`agent-core/src/main/java/com/agent/core/tool/mcp/McpStdioTransport.java` 改为导入 `com.agent.sandbox.mcp.McpStdioProcess`，`agent-sandbox` 中的 Docker 实现直接实现同一 SPI。`agent-sandbox/pom.xml` 保持不增加 `com.agent:agent-core` 依赖；`agent-core/pom.xml` 保持现有对 `agent-sandbox` 的单向依赖，Maven reactor 按该方向先编译 sandbox 再编译 core。除上述 SPI 文件移动与 import 修改外，不新增公共模块或第二份同名 SPI。

`DockerMcpStdioRunner.start(McpDockerLaunchSpec, Map<String,String>)` 使用 docker-java `createContainerCmd`、`withAttachStdin(true)`、`withAttachStdout(true)`、`withAttachStderr(true)`、`withStdinOpen(true)`、`withTty(false)`、`startContainerCmd` 和 `attachContainerCmd(...).withFollowStream(true).withStdIn(...)`。HostConfig 精确设置工作区 bind 的 `AccessMode.ro/rw`、`networkMode=none`、`readonlyRootfs=true`、`memoryBytes`、`nanoCpus`、`pidsLimit`、`privileged=false`；不挂 Docker socket，不使用 host network，不新增 capability。attach 的 `Frame` 必须把 `STDOUT` 与 `STDERR` 分别写入有界管道，禁止把 stderr 混入 JSON-RPC。
