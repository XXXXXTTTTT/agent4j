...
export const CONTENT_WIDTHS = ['WIDE', 'FOCUS'] as const
...
export function normalizeAppearancePreferences(value: unknown): AppearancePreferences {
  ...
  if (!includes(COLOR_MODES, record.colorMode) || !includes(THEME_PRESETS, record.themePreset) || !includes(UI_FONTS, record.font) || !includes(UI_DENSITIES, record.density) || !includes(UI_RADII, record.radius) || (record.contentWidth !== undefined && !includes(CONTENT_WIDTHS, record.contentWidth)) || (record.accentColor !== undefined && record.accentColor !== null && (typeof record.accentColor !== 'string' || !/^#[0-9a-fA-F]{6}$/.test(record.accentColor)))) return DEFAULT_APPEARANCE
  return { colorMode: record.colorMode, themePreset: record.themePreset, font: record.font, density: record.density, radius: record.radius, contentWidth: record.contentWidth === undefined ? 'WIDE' : record.contentWidth, accentColor: record.accentColor === undefined ? null : record.accentColor }
}
