...
export function ModelSettingsDialog({ controller, onClose }: ModelSettingsDialogProps) {
  const [provider, setProvider] = useState({ displayName: '', baseUrl: '', chatCompletionsPath: '/v1/chat/completions', apiKey: '' })
  ...
  return <div className="dialog-backdrop" role="dialog" aria-modal="true" aria-label="模型配置">
    <section className="workspace-dialog model-settings-dialog">
      ...
      <form onSubmit={(event) => void createProvider(event)}><h3>新增 Provider</h3><input aria-label="Provider 名称" placeholder="Provider 名称" value={provider.displayName} onChange={(event) => setProvider({ ...provider, displayName: event.target.value })} /><input aria-label="API Base URL" placeholder="https://gateway.example/v1" value={provider.baseUrl} onChange={(event) => setProvider({ ...provider, baseUrl: event.target.value })} /><input aria-label="Chat Completions 路径" placeholder="/v1/chat/completions" value={provider.chatCompletionsPath} onChange={(event) => setProvider({ ...provider, chatCompletionsPath: event.target.value })} /><input aria-label="API Key" type="password" placeholder="API Key" value={provider.apiKey} onChange={(event) => setProvider({ ...provider, apiKey: event.target.value })} /><button type="submit">保存 Provider</button></form>
