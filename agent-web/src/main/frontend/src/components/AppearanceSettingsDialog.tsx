  ...
  return <div className="appearance-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <aside className="appearance-drawer" role="dialog" aria-modal="true" aria-label="外观设置">
      ...
      <div className="appearance-drawer-content">
        ...
        <section className="appearance-option-group"><h3>强调色</h3><label className="appearance-accent-picker"><input type="color" aria-label="自定义强调色" value={preferences.accentColor ?? '#8ab4ff'} onChange={(event) => updatePreferences({ accentColor: event.target.value })} /><span>{preferences.accentColor ?? '使用预设颜色'}</span><button type="button" className="icon-button" aria-label="恢复预设强调色" title="恢复预设强调色" onClick={() => updatePreferences({ accentColor: null })}><RotateCcw aria-hidden="true" size={14} /></button></label></section>
